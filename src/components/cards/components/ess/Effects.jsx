...
function Effect({
  ...
  vs = variables.map(v => v.demo);

  vs.push('cntry', 'year');

  const groups = vs.filter(v => groupSet.has(v));
  ...
  const summaryinfo = variables.map(item => ({
    demographic: item.demo,
    mrank: item.my_ranks,
    rank: item.my_ranks > per40 ? ' most ' : item.my_ranks <= per30 ? ' least ' : ' middle ',
  }));
  ...
  const isSubGraph = !!groups.find(n => n.toLowerCase() === subGraphId?.toLowerCase());
  return (
    <>
      <div
      ...
      >
        {groups
          .filter(n => (subGraphId ? subGraphId.toLowerCase() === n.toLowerCase() : true))
          .slice(0, 3)
          .map(node =>
            node === 'cntry' ? (
              showMap && <div className={classnames('my-3 col-md-4 col-12 eff')}></div>
            ) : (
              <div key={node} className={classnames('my-3 col-md-4 col-12 eff')}>
                <Effects
                  isBinary={isBinary}
                  shortname={shortname}
                  source={'ESS'}
                  title={
                    data.filter(d => filterData(d, node))[1].demographic.demographics_metum
